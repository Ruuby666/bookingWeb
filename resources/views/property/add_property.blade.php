        ...
        <div class="form-container">
            <form action="{{ isset($property) ? route('properties.update', $property->id) : route('properties.store') }}"
                ...
                <div class="form-item">
                    ...
                    <textarea id="description" name="description" rows="2" required>{{ old('description', $property->description ?? '') }}</textarea>
                </div>
                ...
                @php
                    $booleanFields = ['entertainment', 'parking', 'pool', 'garden', 'safeBox', 'terrace', 'wifi'];
                @endphp
                ...
                @foreach ($booleanFields as $field)
                    <div class="boolean-fields">
                        ...
                        <select id="{{ $field }}" name="{{ $field }}" required>
                            <option value="1" {{ isset($property) && $property->$field ? 'selected' : '' }}>Yes
                            </option>
                            <option value="0" {{ isset($property) && !$property->$field ? 'selected' : '' }}>No
                            </option>
                        </select>
                    </div>
                @endforeach
